...
const gameSlot = async (conn, m, db) => {
    ...
    // Menghasilkan nilai hadiah yang lebih dinamis
    const randomLimit = Math.floor(Math.random() * 5) + 1; // Random limit 1-10
    const randomMoney = Math.floor(Math.random() * 500) + 500; // Hadiah uang acak (500-1000)
    const botNumber = await conn.decodeJid(conn.user.id);

    try {
        // Probabilitas menang dikurangi, misalnya 20% kemungkinan menang
        const isWin = Math.random() < 0.1; // 20% kemungkinan menang

        if (isWin) {
            // Kemenangan, pastikan simbol slot sama
            const winningSymbol = pickRandom(sotoy);
            const listSlot2Win = `${winningSymbol} : ${winningSymbol} : ${winningSymbol}`;

            db.users[m.sender].limit -= 1;
            db.set[botNumber].limit += 1;

            // Memberikan hadiah acak (limit dan uang)
            db.users[m.sender].limit += randomLimit;
            db.users[m.sender].uang += randomMoney;

            let sloth = `[  🎰VIRTUAL SLOT 🎰  ]\n------------------------\n\n${listSlot1}\n${listSlot2Win} <=====\n${listSlot3}\n\n------------------------\n[  🎰 VIRTUAL SLOT 🎰  ]\n\n*Keterangan* :\n_You Win🎉_ <=====Limit + ${randomLimit}, Uang + ${randomMoney}`;
            conn.sendMessage(m.chat, { text: sloth }, { quoted: m });
        } else {
            // Kalah, hasil slot tetap acak
            db.users[m.sender].limit -= 1;
            db.set[botNumber].limit += 1;
